...
async def get_carrello(request: Request):
    carrello = request.session.get("carrello", [])
    return JSONResponse(carrello)

# Endpoint per aggiornare la quantità
@app.post("/carrello/aggiorna/{prodotto_id}")
async def aggiorna_carrello(request: Request, prodotto_id: int):
    try:
        ...
        for item in carrello:
            if item["id"] == prodotto_id:
                item["quantita"] = max(1, int(quantita))
                break
...
@app.get("/api/product_page")
async def get_products():
    conn = connessione()
    cursor = conn.cursor(dictionary=True)
    try:
        cursor.execute("""
            SELECT id, nome AS name, prezzo AS price,
                   prezzo_originale AS original_price,
                   immagine AS image, categoria AS category
            FROM prodotti
            WHERE quantita > 0 AND categoria = 'Computer Accessories'
        """)
    ...
    try:
        cursor.execute("""
            SELECT id, nome AS name, prezzo AS price,
                   ...
                   immagine AS image, categoria AS category
            FROM prodotti
            WHERE quantita > 0 AND prezzo < prezzo_originale
            ORDER BY (prezzo_originale - prezzo) DESC
            LIMIT 8
        """)
        hot_products = cursor.fetchall()
        return JSONResponse({"status": "success", "products": hot_products})
    except Exception as e:
        return JSONResponse({"status": "error", "message": str(e)}, status_code=500)
    finally:
        cursor.close()
